									...
									>
										<div style="clear: both">
											<?php echo $this->Form->input('ObiProduct.search_phrase', array(
												'type' => 'text',
												'label' => false,
												'rds-list-filter' => 'ObiProduct.name',
												'rds-list-filter-value' => 'searchPhrase',
												'ng-model' => 'searchPhrase',
												'ng-keypress' => 'search(searchPhrase)',
												'class' => 'input_search',
										...
										<table ng-show="products" app-table-sort="orderBy">
											...
											<tbody>
												<tr class="product_row" ng-repeat="product in products | orderBy:orderBy">
													<td>
														<img src="/files/products/{{product.ObiProductMedia.0.src}}" onerror="this.onerror=null;this.src='/img/no-image.png';" />
													</td>
													<td>{{product.ObiProduct.name}}</td>
													<td>{{product.ObiProductDataBasics.product_ean}}</td>
													<td>
														<span>
															<em >
																{{product.ObiProductCategory.ObiCategory.name}}
															</em>
														</span>
													</td>
													<td><?php echo $this->Html->link('<i class="fa fa-search" aria-hidden="true"></i> zobacz', '/products/p/{{product.ObiProduct.id}}', array('escape' => false, 'escapeUrl' => false)); ?></td>
												</tr>
											</tbody>
										</table>
									</div>
