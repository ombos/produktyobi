...
class PHPWord_Section_Image {
	...
	public function __construct($src, $style = null, $isWatermark = false) {
		...
		if (isset($inf['extension'])) {
			...
			$docImgFile = $_SERVER['DOCUMENT_ROOT'].'/s/app/webroot/files/docs/images/'.$inf['basename'];
			
			file_put_contents($docImgFile, file_get_contents($src));
			...
			$docImgFile = $_SERVER['DOCUMENT_ROOT'].'/s/app/webroot/files/docs/images/';

			$imageBlob = explode('data:image/jpeg;base64,', $src);
			
			$imageRanomName = $this->generateRandomString(15);
			
			file_put_contents($docImgFile.$imageRanomName.'.jpg', 
				array(
					'dirname' => base64_decode($imageBlob[1]),
					'extension' => 'jpg'
				));
			
			$src = $docImgFile.$imageRanomName.'.jpg';
			$inf['extension'] = $ext = $this->_extension = 'jpg';
